...
const allValues = [...values, Number(ucl), Number(cl), Number(lcl)].filter((v) => Number.isFinite(v));
...
const x =
data.length === 1 ? padding + innerWidth / 2 : padding + (i * innerWidth) / (data.length - 1);
...
const lineY = (value) => padding + getYPosition(Number(value), minVal, maxVal, innerHeight);

return ( <div style={styles.card}> <h3 style={styles.cardTitle}>{title}</h3>
<svg viewBox={`0 0 ${width} ${height}`} style={{ width: "100%", height: "auto", background: "#fff" }}> <rect x="0" y="0" width={width} height={height} fill="#ffffff" stroke="#e5e7eb" />
...
```
    ...
    <line x1={padding} y1={lineY(ucl)} x2={width - padding} y2={lineY(ucl)} stroke="#dc2626" strokeDasharray="6 4" strokeWidth="2" />
    <line x1={padding} y1={lineY(cl)} x2={width - padding} y2={lineY(cl)} stroke="#16a34a" strokeDasharray="6 4" strokeWidth="2" />
    <line x1={padding} y1={lineY(lcl)} x2={width - padding} y2={lineY(lcl)} stroke="#dc2626" strokeDasharray="6 4" strokeWidth="2" />

    <text x={width - 110} y={lineY(ucl) - 6} fontSize="12" fill="#dc2626">
      UCL {ucl}
    </text>
    <text x={width - 110} y={lineY(cl) - 6} fontSize="12" fill="#16a34a">
      CL {cl}
    </text>
    <text x={width - 110} y={lineY(lcl) - 6} fontSize="12" fill="#dc2626">
      LCL {lcl}
    </text>

    {points.length > 1 && <polyline fill="none" stroke={color} strokeWidth="3" points={polyline} />}

    {points.map((p, i) => (
      <g key={i}>
        <circle cx={p.x} cy={p.y} r="4" fill={color} />
        <text x={p.x} y={height - 10} textAnchor="middle" fontSize="11" fill="#374151">
          {p.label}
        </text>
...
```
  return {
    ...
    xbar,
    r,
    xbarStatus: xbar > Number(xbarUCL) || xbar < Number(xbarLCL) ? "超限" : "正常",
    rStatus: r > Number(rUCL) || r < Number(rLCL) ? "超限" : "正常",
  };
});
```
...
setGroups((prev) => [...prev, { values: nums, time: formatDateTime() }]);
setInputs(["", "", "", "", ""]);
```
...
return ( <div style={styles.page}> <div style={styles.container}> <h1 style={styles.title}>SPC Xbar-R 质量监控</h1>
    ...
    <div style={styles.limitGrid}>
      <div style={styles.card}>
        ...
        <div style={styles.limitRow}>
          ...
          <input value={xbarUCL} onChange={(e) => setXbarUCL(e.target.value)} style={styles.smallInput} />
        </div>
        <div style={styles.limitRow}>
          <label style={styles.label}>CL</label>
          <input value={xbarCL} onChange={(e) => setXbarCL(e.target.value)} style={styles.smallInput} />
        </div>
        <div style={styles.limitRow}>
          <label style={styles.label}>LCL</label>
          <input value={xbarLCL} onChange={(e) => setXbarLCL(e.target.value)} style={styles.smallInput} />
        </div>
      </div>
      ...
      <div style={styles.card}>
        ...
        <div style={styles.limitRow}>
          <label style={styles.label}>UCL</label>
          <input value={rUCL} onChange={(e) => setRUCL(e.target.value)} style={styles.smallInput} />
        </div>
        <div style={styles.limitRow}>
          <label style={styles.label}>CL</label>
          <input value={rCL} onChange={(e) => setRCL(e.target.value)} style={styles.smallInput} />
        </div>
        <div style={styles.limitRow}>
          <label style={styles.label}>LCL</label>
          <input value={rLCL} onChange={(e) => setRLCL(e.target.value)} style={styles.smallInput} />
        </div>
      </div>
    </div>
            ...
            {processedData.length === 0 ? (
              ...
              processedData.map((row) => (
                <tr key={row.id}>
                  ...
                  <td style={styles.td}>{row.values.map((v) => toFixedNum(v)).join(" / ")}</td>
